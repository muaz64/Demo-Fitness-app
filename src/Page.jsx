...
export default function Page() {
 ...
 const fetchAIAdvice = async (bmiValue) => {
  try {
    const response = await fetch("/api/fitness-advice", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ age, gender, goal, bmi: bmiValue }),
    });
      ...
      <Card className="max-w-md mx-auto mt-6 p-6 bg-gray-800/80 backdrop-blur rounded-2xl shadow-lg text-black">
        <CardContent>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            {aiAdvice && (
              <>
                <h3 className="text-lg font-semibold mt-4">AI-Powered Advice:</h3>
